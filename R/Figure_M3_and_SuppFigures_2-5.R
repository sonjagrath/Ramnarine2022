################################################################################
# Figure_M3_and_SuppFigures_2-5.R
################################################################################

## Author: Timothy Ramnarine
## Input:
### L2FC_GEI_rec90_v_MSB.xlsx
### L2FC_rec90_v_MSB.xlsx
### GeneGroups.xlsx
## Last update: 2021-05-17 by Sonja Grath
...
geiplot <-  ggplot(gei, aes(x = Name , y = L2FC)) +
  ...
  geom_text(label = gei$psimple_msb, y = 4.7, 
            angle = 0,
            nudge_x = -0.15,  
            size = 4) +
  geom_text(label = gei$psimple_rec90, y = 4.7, 
            angle = 0,
            nudge_x = 0.15, 
            size = 4) +
...
p1 <-  ggplot(subv, aes(x = Gene_name , y = L2FC)) +
  ...
  scale_y_continuous(limits = c(-2, 8.2),
                     breaks = c(-2, -1, 0, 1, 2, 3, 4, 5, 6, 7, 8)) +
  geom_text(label = subv$psimple_msb, y = 8, 
            angle = 0,
            nudge_x = -0.15,  
            size = 4) +
  geom_text(label = subv$psimple_rec90, y = 8, 
            angle = 0,
            nudge_x = 0.15, 
            size = 4) +
  scale_color_manual(values = c("#EB9622", "#4F17C6")) +
...
p2<- ggplot(subnv, aes(x = Gene_name, y = L2FC)) +
  ...
  scale_y_continuous(limits = c(-2, 8.2),
                     breaks = c(-2, -1, 0, 1, 2, 3, 4, 5, 6, 7, 8)) +
  geom_text(label = subnv$psimple_msb, y = 8, 
            angle = 0,
            nudge_x = -0.15,  
            size = 4) +
  geom_text(label = subnv$psimple_rec90, y = 8, 
            angle = 0,
            nudge_x = 0.15, 
            size = 4) +
  scale_color_manual(values = c("#EB9622", "#4F17C6")) +
...
plotall <-  ggplot(l2fc_rec90MSB, aes(factor(Gene_name), L2FC)) + 
  geom_bar(aes(x = as.factor(Gene_name), y = L2FC, 
               fill = as.factor(Dataset)), 
           stat = "identity", 
           width = 0.9) +
  theme(panel.background = element_blank()) + 
  scale_fill_manual(values = c ("#EB9622", "#4F17C6"), 
                    labels = c("MSB", "rec90"), 
                    name = "") +
...
plot1_2 <- ggpubr::ggarrange(p1a, p2a, 
                             nrow = 1, 
                             ncol = 2, 
                             legend.grob = legend, 
                             legend = "bottom", 
...
# Main Figure 4 & Supplemental Figures 4 and 5
...
mtnplot <- ggplot(mtn ,aes(factor(Name), MSB_LFC)) + 
  geom_bar(aes(x = as.factor(Name), y = MSB_LFC, 
...
bariplot <- ggplot(bari_jheh, aes(factor(Name), MSB_LFC)) + 
  ...
  geom_text(label = bari_jheh$psimple_msb, y = 1.7, 
            angle = 0, 
            size = 5) +
  scale_fill_manual(values = c ("#549422"), 
                    labels = c("Epoxide Hydrolase")) +
  labs(x = "", y = "Log2(stress/control)")+
  theme(text = element_text(size = 10)) +
  theme(axis.text.x = element_text(size = 10, 
                                   angle = 45, 
